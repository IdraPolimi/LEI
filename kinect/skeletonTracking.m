...
% disp('Loading robot params for vrep sim...');
% loadRobotParam
% disp('OK\n\n');
% disp('Connecting to vrep robot...');
...
% disp('OK\n\n');
% 
% disp('Retrieving handles from robot...');
% for jj = 1:3
...
while(Pos(1)>0 && zz < 1000)
    ...
    leftShoulder = [x3D_(3),y3D_(3), z3D_(2)] - [x3D_(2),y3D_(2), z3D_(2)];
    ...
    yList1(zz)= y(2);
    yList2(zz)= y(3);
    %computeAngle(leftUpperArm, leftShoulder)*180/pi
    
    %disp('lsPitch************lsRoll************leRoll');
    %disp([num2str(y(1)*180/pi),'************',num2str(y(2)*180/pi),'************',num2str(y(3)*180/pi),'************']);
    %computeAngle(leftUpperArm, leftLowerArm)*180/pi
